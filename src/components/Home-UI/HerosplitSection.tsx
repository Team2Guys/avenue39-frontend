...
const HeroSplitSection: React.FC<HeroSplitSectionProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`bg-white/80 flex flex-col justify-center items-start p-3 sm:p-10 lg:p-12 sm:h-[550px] mt-5  ${reverse ? "sm:mr-5" : "sm:ml-5"} w-[343px] h-[196px] sm:w-[55%] md:w-[40%] xl:w-[598px] xl:h-[761px] text-start lg:text-left`}
        >
          <h2 className="text-[20px] sm:text-2xl  xl:text-[33px] font-light font-alethia leading-tight">
            {title}
          </h2>
          {subtitle && <p className="text-[23px] mt-2">{subtitle}</p>}
          <Link
            href={buttonLink}
